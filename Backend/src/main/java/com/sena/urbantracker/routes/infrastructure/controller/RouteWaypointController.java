...
public class RouteWaypointController extends BaseController<RouteWaypointReqDto, RouteWaypointResDto, Long> {
    ...
    public RouteWaypointController(ServiceFactory serviceFactory) {
    ...
    public ResponseEntity<CrudResponseDto<List<RouteWaypointReqDto>>> findByRouteId(@PathVariable Long routeId) {
        CrudResponseDto<List<RouteWaypointReqDto>> response =
                serviceFactory.createSpecializedService(EntityType.ROUTE_WAYPOINT, com.sena.urbantracker.routes.application.service.RouteWaypointService.class)

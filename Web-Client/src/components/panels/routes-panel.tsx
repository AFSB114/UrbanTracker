...
export function RoutesPanel({ showTitle = false }: { showTitle?: boolean }) {
  // Estado para saber qué ruta está seleccionada (null = ninguna)
    ...
    <div className="h-full overflow-y-auto px-2 py-2">
      ...
      {showTitle && (
        <div className="flex items-center gap-2 mb-4">
          {selected !== null && (
            <button
              className="flex items-center gap-1 text-gray-500 hover:text-blue-600 text-sm font-medium px-2 py-1 rounded transition focus:outline-none focus:ring-2 focus:ring-blue-200 bg-transparent border-none cursor-pointer"
      ...
      {selected === null ? (
        <div className="space-y-3">
          {ROUTES.map((route, idx) => (
            ...
            <div
              key={route.name}
              className="bg-white text-gray-900 p-4 rounded-xl w-full font-sans border border-gray-300 flex flex-col shadow-sm transition-all duration-200 hover:shadow-xl hover:scale-[1.02] cursor-pointer"
              onClick={() => setSelected(idx)}
            >
                 <div className="flex items-center justify-between mb-2">
                   <div className="flex flex-col">
                     <h4 className="text-base font-semibold leading-tight text-gray-900">{route.name}</h4>
                     <p className="text-xs text-gray-700 leading-tight">{route.description}</p>
                   </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-green-500 rounded-full border border-gray-200" />
                ...
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-red-500 rounded-full border border-gray-200" />
